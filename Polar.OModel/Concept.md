...
Уже пора писать комментарии. Проект я назвал Polar.Universal, в нем буду определять новую UniversalSequence, новый UniversalIndex, также UniversalKeyIndex и UniversalVectorIndex. 

Работаю над созданием UniversalKeyIndex. У него смысл аргументов конструктора несколько другой:
```
...
```
Есть ключевая функция, определенная на объекте элемента последовательности, вырабатывает объект, удовлетворяющий интерфейсу IComparable.  Для этого объекта может быть определена функция hashOfKey. Ключевая функция определена на всех элеменатах последовательности. 
